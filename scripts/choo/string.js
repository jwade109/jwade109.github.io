...
// KEPT BECAUSE THIS IS A TERRIBLE TRAIN SIMULATION,
// BUT IT'S ACTUALLY A REALLY GOOD ROPE SIMULATION
...
function Train(cars)
{
    ...
    for (let i = 0; i < Math.random() * 70 + 200; ++i)
    {
...
Train.prototype.constrain = function(path, ctx)
{
    for (let i = 0; i < this.cars.length; ++i)
    {
        let c = this.cars[i];
        let [p, t] = path.nearestPoint(c.pos);
        ...
        let diff = sub2d(p, c.pos);

        c.pos = add2d(c.pos, mult2d(diff, 0.7));
    }
}
